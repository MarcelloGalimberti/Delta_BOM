...
st.title('Delta BOM D33 vs DAA rev.20240129')
...
PLM_BOM['Gruppo Tecnico']=[stringa[:2] for stringa in PLM_BOM['Gruppo Tecnico'].astype(str)]
...
#sap_delta = sum_SAP_2_tot[sum_SAP_2_tot.Articolo=='76410221A']

#plm_delta = sum_PLM_2_tot[sum_PLM_2_tot.Articolo=='76410221A']
...
#delta_qty_liv_2_1 = sum_SAP_2.merge(sum_PLM_2, how='outer',left_on='key',right_on='key',indicator=True,
...
#delta_qty_table = delta_qty_liv_2_1[delta_qty_liv_2_1['Delta_qty']!=0]
...
#delta_qty_table['check'] = None

#codici_sap = list(delta_qty_table.Articolo_SAP)

#codici_plm = list(delta_qty_table.Articolo_PLM)





#for i in range(len(delta_qty_table)):

    #if (delta_qty_table.Articolo_SAP.astype(str).iloc[i]!='nan') and (delta_qty_table.Articolo_SAP.iloc[i] in codici_sap) and (delta_qty_table.Articolo_SAP.iloc[i] in codici_plm)

    #if (delta_qty_table.Articolo_SAP.iloc[i] in codici_plm):

       # delta_qty_table['check'].iloc[i]='x'



#delta_qty_table = delta_qty_table[delta_qty_table.check == 'x']



#delta_qty_table.drop(columns=['_merge','key','check'], inplace=True)
...
custom_compare['check_STGR']=custom_compare['STGR_PLM']==custom_compare['STGR_SAP']

custom_compare['check_STGR_desc']=custom_compare['Descrizione Sottogruppo_PLM']==custom_compare['Descrizione Sottogruppo_SAP']
