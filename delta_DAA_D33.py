...
# Elaboro file Cap D33 (identica alla versione precedente)
...
#PLM_BOM = importa_plm(PLM)     modifica 20231228
...
if livello_1 == 'M':
    L_BOM_input = SAP_M[['Liv.','Articolo','Qty']] 
    R_BOM_input = PLM_M[['Liv.','Articolo','Qty']] 
elif livello_1 == 'V':
    L_BOM_input = SAP_V[['Liv.','Articolo','Qty']] 
    R_BOM_input = PLM_V[['Liv.','Articolo','Qty']] 
else:
    L_BOM_input = SAP_X[['Liv.','Articolo','Qty']] 
    R_BOM_input = PLM_X[['Liv.','Articolo','Qty']] 
...
df_PLM_descrizione = PLM_BOM[['Articolo','Testo breve oggetto']]  #[['Item Id','Rev Name']]
...
#AIR_PLM = AIR_PLM.drop(columns=['Articolo'])
AIR_PLM = AIR_PLM[['Articolo','Testo breve oggetto','Esito check codice','Azione','Responsabile','Due date','Status']]
...
for codice in codici_grafo:
    ...
    AIR_SAP_sub = df_output_confronto_L.merge(SAP_M,how='left',left_on='Articolo SAP', right_on='Articolo')
...
df_output_confronto_R.rename(columns= {'Liv.':'Liv. PLM','Articolo':'Articolo PLM','Qty':'Qty PLM'},inplace = True)
# aggiunge descrizione
AIR_PLM_sub = df_output_confronto_R.merge(df_PLM_descrizione,how = 'left',left_on='Articolo PLM', right_on='Articolo')
AIR_PLM_sub = AIR_PLM_sub.drop(columns='Articolo')
AIR_PLM_sub = AIR_PLM_sub[['Liv. PLM','Articolo PLM','Testo breve oggetto','Qty PLM']]
AIR_PLM_sub.rename(columns={'Testo breve oggetto':'Descrizione PLM'},inplace=True)
AIR_PLM_sub.drop_duplicates(inplace=True)
