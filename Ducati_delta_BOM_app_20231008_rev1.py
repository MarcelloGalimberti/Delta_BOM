...
st.title('ASC MTSV4 MTO Delta BOM rev.20231008')
...
def importa_cap (df):
    ...
    df = df.drop(columns=['Gruppo Tecnico','Descr. Gruppo Tecnico','Liv. esplosione',
                           'Testo breve oggetto','Unità mis.componente','Peso netto',
                           'Unità di peso','Ril.Prod.','Ril.Tecn.',
                           'Ril.Ric.','Testo posizione riga 1','Inizio validità',
                           'Fine validità','Numero di modifica','N. modifica fino a',
                           'Testo posizione riga 2'])
    df = df[['Liv.','Materiale','Qtà comp. (UMC)','MerceSfusa (BOM)']]
    df.rename(columns={'Materiale':'Articolo','Qtà comp. (UMC)':'Qty'},inplace=True)
    df = df.fillna(0)
    df['MerceSfusa (BOM)'] = df['MerceSfusa (BOM)'].replace('X',1)
