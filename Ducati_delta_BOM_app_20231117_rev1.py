...
st.title('ASC MTSV4 MTO Delta BOM rev.20231117')
...
# Cancellare albero motore da PLM (individuare indici inizio e fine)
# modificare struttura secondo logica (studiare algo)
# Aggiungere alboro modificato a file PLM (può essere accodato)
...
#st.write('albero notore processato prima drop', albero_motore_processato)
...
# eliminare albero motore non processato da PLM_BOM
#PLM_BOM.drop(PLM_BOM.index[range(indice_inizio, indice_fine)], axis=0, inplace=True) # elimina compresi [range(0, n)]
...
#st.write('albero notore processato', albero_motore_processato)
...
st.subheader('Tabella delta', divider='red')
st.write(df_delta)
